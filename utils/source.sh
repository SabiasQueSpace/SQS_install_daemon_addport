...
if [[ ("$precompiled" == "true") ]]; then
	read -r -e -p "Paste the github link coin precompiled *.tar.gz: " coin_precompiled
	echo
else
...
if [[ ("$precompiled" == "true") ]]; then
	if [ -d "$HOME/utils/daemon_builder/temp_coin_builds/${coindir}/${repzipcoin}/bin" ]; then
		PRECOMPILEDBIN="$HOME/utils/daemon_builder/temp_coin_builds/${coindir}/${repzipcoin}/bin"
	else
		PRECOMPILEDBIN="$HOME/utils/daemon_builder/temp_coin_builds/${coindir}/${repzipcoin}"	
	fi
	cd $PRECOMPILEDBIN
else
	cd $HOME/utils/daemon_builder/temp_coin_builds/${coindir}/src/
...
if [[ ("$precompiled" == "true") ]]; then
	sudo strip $PRECOMPILEDBIN/${coind}
	sudo cp $PRECOMPILEDBIN/${coind} /usr/bin
	
	if [[ ("$ifcoincli" == "y" || "$ifcoincli" == "Y") ]]; then
		sudo strip $PRECOMPILEDBIN/${coincli}
		sudo cp $PRECOMPILEDBIN/${coincli} /usr/bin
	fi

	if [[ ("$ifcointx" == "y" || "$ifcointx" == "Y") ]]; then
		sudo strip $PRECOMPILEDBIN/${cointx}
		sudo cp $PRECOMPILEDBIN/${cointx} /usr/bin
	fi

	if [[ ("$ifcoingtest" == "y" || "$ifcoingtest" == "Y") ]]; then
		sudo strip $PRECOMPILEDBIN/${coingtest}
		sudo cp $PRECOMPILEDBIN/${coingtest} /usr/bin
	fi

	if [[ ("$ifcointools" == "y" || "$ifcointools" == "Y") ]]; then
		sudo strip $PRECOMPILEDBIN/${cointools}
		sudo cp $PRECOMPILEDBIN/${cointools} /usr/bin
	fi
else
